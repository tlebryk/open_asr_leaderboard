...
# Apache-2.0).
"""Reference rendering agreement: site extraction, classification and scoring.

A **site** is a span of a raw reference transcript that the leaderboard's text
normalizer rewrites, so the reference's rendering and the normalizer's rendering
of the same words score identically under WER. Casing, punctuation, en-GB/en-US
spelling, honorific abbreviations, pointed acronyms and digits-versus-words are
all such rewrites.

For each site a hypothesis is **eligible** when its normalized text reproduces
the site's normalized words (the model got the words right, so only the
rendering is in question), and it **agrees** when its raw text at that span is
character-for-character the reference's raw span. The agreement rate is the share
of eligible sites at which a model agrees.
...
Public entry points: :func:`extract_sites` and :func:`keep_site` build a clip's
site list, :func:`score_clip` scores one hypothesis against it, and
...
def spelling_map() -> dict[str, str]:
    """The en-GB to en-US map the normalizer applies, used to label sites."""
    from normalizer.english_abbreviations import english_spelling_normalizer
...
def extract_sites(ref: str) -> list[tuple[str, str, str, int, int, int, int]]:
    """Every span of ``ref`` the normalizer rewrites.

    Each site is ``(raw_span, norm_span, cls, raw_lo, raw_hi, full_lo,
...
BLOCKED_SPELLINGS = frozenset(
    {
        ...
        "curb",  # kerb: the verb "to curb" is spelled the same in en-GB
        "curbs",
...
# Spelling sites blocked by their RAW (reference-side) form. These fail the
...
def keep_site(site) -> bool:
    """Whether a site survives the frozen exclusions."""
    raw, norm, cls = site[0], site[1], site[2]
...
def sites_for(ref: str) -> tuple[tuple, tuple[str, ...]]:
    """Retained sites of ``ref`` plus its normalized words.

    ``ref`` must have its whitespace collapsed already, so that manifests that
    differ only in whitespace share a cache entry.
    """
    sites = tuple(s for s in extract_sites(ref) if keep_site(s))
    return sites, tuple(nfull(ref))
...
def score_clip(sites, ref_full, hyp: str) -> list[tuple[int, bool, bool, str]]:
    """Score one hypothesis at every site of one clip.

    Returns ``(site_index, eligible, agreed, hyp_raw_span)`` per site. A site is
    ...
    res = []
    for si, (raw_span, norm_span, cls, lo, hi, fa, fb) in enumerate(sites):
        if fa < 0 or fb <= fa:
...
def score_pairs(pairs) -> dict:
    ...
    class. Clips with no retained site contribute nothing.
    """
    v1 = [0, 0]
    v2 = [0, 0]
    by_class = {cls: [0, 0] for cls in REPORTED_CLASSES}
    for ref, hyp in pairs:
        sites, ref_full = sites_for(" ".join(ref.split()))
        if not sites:
            continue
        for (_, eligible, agreed, _), site in zip(score_clip(list(sites), list(ref_full), hyp), sites):
            if not eligible:
                continue
            v1[1] += 1
            v1[0] += agreed
