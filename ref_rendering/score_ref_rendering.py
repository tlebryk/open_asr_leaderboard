...
pointed acronyms, digits versus number words. Each such span is a *site* whose
two renderings score identically under WER. For each site where a model's
normalized output reproduces the site's words, this script asks whether its raw
output reproduces the reference's exact rendering.
